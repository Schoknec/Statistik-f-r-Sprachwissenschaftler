...
Wir sehen, dass das Intercept für jedes `subject` und `scenario` anders ist, aber die Koeffizienten für die Anstiege (`genderM` und `attitudepol`) überall gleich sind und zwar die gleichen, die es als fixe Effekte gibt!  Bei dem Modell mit Anstieg in der RE-Struktur gibt es einen Anstieg für jedes `subject` und `scenario`.
